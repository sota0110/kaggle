...
# カラムの抽出
df_exctracted = df[['Pclass', 'Fare', 'Embarked', 'Embarked_C', 'Mrs', 'Miss', 'Master', 'Survived']]

# 欠損値の補完
si = SimpleImputer(missing_values=np.nan, strategy='mean')
df_exctracted = si.fit_transform(df_exctracted)
df_exctracted = pd.DataFrame(df_exctracted, columns=['Pclass', 'Fare', 'Embarked', 'Embarked_C', 'Mrs', 'Miss', 'Master', 'Survived'])

# 説明変数と目的変数の分割
X = df_exctracted[['Pclass', 'Fare', 'Embarked', 'Embarked_C', 'Mrs', 'Miss', 'Master']]
...
df_result.to_csv('titanic/data/result_lr_reducealot.csv', index=False)